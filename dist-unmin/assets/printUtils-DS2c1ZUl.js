import { t as toDisplayFieldName } from "./jspdf.es.min-iF735MOB.js";
import "./vendor-ui-CLSr_GWM.js";
import "./EntryViewDialog-p97oVngN.js";
import "./index-CT7EzYyk.js";
...
const printEntries = (entries, options = {}) => {
  const {
...
const printSingleEntry = (entry, options = {}) => {
  printEntries([entry], options);
};
const detectBookList = (tableData) => {
  const columnNames = tableData.columns.map((col) => col.name.toLowerCase());
  const hasBook = columnNames.some(
    (name) => name.includes("book") || name.includes("title") || name.includes("novel")
  );
  const hasAuthor = columnNames.some(
    (name) => name.includes("author") || name.includes("writer") || name.includes("by")
  );
  const hasReadingRelated = columnNames.some(
    (name) => name.includes("read") || name.includes("genre") || name.includes("isbn") || name.includes("publisher") || name.includes("pages") || name.includes("rating")
  );
  return hasBook && hasAuthor || hasBook && hasReadingRelated;
};
const detectMovieList = (tableData) => {
  const columnNames = tableData.columns.map((col) => col.name.toLowerCase());
  const hasMovie = columnNames.some(
    (name) => name.includes("movie") || name.includes("film") || name.includes("title")
  );
  const hasMovieRelated = columnNames.some(
    (name) => name.includes("director") || name.includes("genre") || name.includes("year") || name.includes("rating") || name.includes("watched") || name.includes("runtime")
  );
  return hasMovie && hasMovieRelated;
};
const detectRecipeList = (tableData) => {
  const columnNames = tableData.columns.map((col) => col.name.toLowerCase());
  const hasRecipe = columnNames.some(
    (name) => name.includes("recipe") || name.includes("dish") || name.includes("meal")
  );
  const hasRecipeRelated = columnNames.some(
    (name) => name.includes("ingredient") || name.includes("cooking") || name.includes("prep") || name.includes("time") || name.includes("servings") || name.includes("cuisine")
  );
  return hasRecipe || hasRecipeRelated && columnNames.some((name) => name.includes("name") || name.includes("title"));
};
const detectInventoryList = (tableData) => {
  const columnNames = tableData.columns.map((col) => col.name.toLowerCase());
  const hasInventory = columnNames.some(
    (name) => name.includes("inventory") || name.includes("stock") || name.includes("warehouse")
  );
  const hasInventoryRelated = columnNames.some(
    (name) => name.includes("quantity") || name.includes("location") || name.includes("sku") || name.includes("barcode") || name.includes("storage") || name.includes("shelf")
  );
  return hasInventory || hasInventoryRelated;
};
const detectShoppingList = (tableData) => {
  const columnNames = tableData.columns.map((col) => col.name.toLowerCase());
...
const detectContactList = (tableData) => {
  const columnNames = tableData.columns.map((col) => col.name.toLowerCase());
...
const detectFinancialRecord = (tableData) => {
  const columnNames = tableData.columns.map((col) => col.name.toLowerCase());
...
const detectTaskList = (tableData) => {
  const columnNames = tableData.columns.map((col) => col.name.toLowerCase());
...
const renderBookListCard = (tableData, title) => {
  const titleCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("title") || col.name.toLowerCase().includes("book") || col.name.toLowerCase().includes("name")
  );
  const authorCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("author") || col.name.toLowerCase().includes("writer")
  );
  const genreCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("genre") || col.name.toLowerCase().includes("category")
  );
  const ratingCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("rating") || col.name.toLowerCase().includes("score")
  );
  const statusCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("read") || col.name.toLowerCase().includes("status") || col.name.toLowerCase().includes("finished")
  );
  const pagesCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("pages") || col.name.toLowerCase().includes("length")
  );
  const yearCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("year") || col.name.toLowerCase().includes("published")
  );
  const totalBooks = tableData.rows.length;
  const readBooks = tableData.rows.filter(
    (row) => statusCol ? row[statusCol.id] === true || String(row[statusCol.id]).toLowerCase().includes("read") : false
  ).length;
  return `
...
const renderMovieListCard = (tableData, title) => {
  const titleCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("title") || col.name.toLowerCase().includes("movie") || col.name.toLowerCase().includes("film")
  );
  const directorCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("director")
  );
  const genreCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("genre") || col.name.toLowerCase().includes("category")
  );
  ...
  const watchedCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("watched") || col.name.toLowerCase().includes("seen")
  );
  const yearCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("year") || col.name.toLowerCase().includes("released")
  );
  return `
...
const renderRecipeListCard = (tableData, title) => {
  const nameCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("name") || col.name.toLowerCase().includes("recipe") || col.name.toLowerCase().includes("dish")
  );
  const cuisineCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("cuisine") || col.name.toLowerCase().includes("type")
  );
  const timeCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("time") || col.name.toLowerCase().includes("duration")
  );
  const servingsCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("serving") || col.name.toLowerCase().includes("portion")
  );
  const difficultyCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("difficulty") || col.name.toLowerCase().includes("level")
  );
  return `
...
const renderInventoryListCard = (tableData, title) => {
  const itemCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("item") || col.name.toLowerCase().includes("product") || col.name.toLowerCase().includes("name")
  );
  const quantityCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("quantity") || col.name.toLowerCase().includes("stock") || col.name.toLowerCase().includes("count")
  );
  const locationCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("location") || col.name.toLowerCase().includes("shelf") || col.name.toLowerCase().includes("warehouse")
  );
  const skuCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("sku") || col.name.toLowerCase().includes("code") || col.name.toLowerCase().includes("id")
  );
...
const renderShoppingListCard = (tableData, title) => {
  const itemCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("item") || col.name.toLowerCase().includes("product") || col.name.toLowerCase().includes("name")
  );
  const quantityCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("quantity") || col.name.toLowerCase().includes("qty")
  );
  const priceCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("price") || col.name.toLowerCase().includes("cost")
  );
  const purchasedCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("purchased") || col.name.toLowerCase().includes("bought") || col.name.toLowerCase().includes("done")
  );
  const categoryCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("category") || col.name.toLowerCase().includes("type")
  );
  const notesCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("notes") || col.name.toLowerCase().includes("comment")
  );
  const totalItems = tableData.rows.length;
  const purchasedItems = tableData.rows.filter(
    (row) => purchasedCol ? row[purchasedCol.id] === true : false
  ).length;
...
const renderContactListCard = (tableData, title) => {
  const nameCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("name") || col.name.toLowerCase().includes("contact")
  );
  const phoneCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("phone") || col.name.toLowerCase().includes("mobile")
  );
  const emailCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("email") || col.name.toLowerCase().includes("mail")
  );
  const companyCol = tableData.columns.find(
    (col) => col.name.toLowerCase().includes("company") || col.name.toLowerCase().includes("organization")
  );
  return `
...
const contentTypeDefinitions = [
  ...
  },
  {
    ...
    renderer: (tableData, title) => renderGenericTable(tableData)
  },
  {
    type: "task-list",
    priority: 70,
...
const detectContentType = (tableData) => {
...
const renderGenericTable = (tableData, title) => {
  return `
...
const renderFieldValue = (key, value, fieldDefinitions, template = "auto") => {
  ...
  if (value && typeof value === "object" && "columns" in value && "rows" in value) {
    ...
    if (template === "auto") {
      ...
      if (detectShoppingList(tableData)) {
        return renderShoppingListCard(tableData);
      } else if (detectContactList(tableData)) {
        return renderContactListCard(tableData);
      }
    }
    return renderGenericTable(tableData);
  }
  if (Array.isArray(value)) {
    if (value.length === 0) {
      return '<span class="text-muted">No items</span>';
    }
    const isImageArray = value.some(
      (item) => typeof item === "string" && (item.includes("blob:") || item.match(/\.(jpg|jpeg|png|gif|webp|svg)$/i))
    );
    if (isImageArray) {
      return `
...
const renderTableCellValue = (value, columnType) => {
  if (value === null || value === void 0 || value === "") {
    return "—";
  }
  switch (columnType) {
...
const generatePrintHTML = (entries, options) => {
  ...
  const headerHTML = `
    <div class="print-header">
      ...
      <div class="print-date">Generated on ${(/* @__PURE__ */ new Date()).toLocaleDateString()} at ${(/* @__PURE__ */ new Date()).toLocaleTimeString()}</div>
    </div>
...
const openPrintPreview = (entries, options = {}) => {
...
const printDocumentHtml = (title, htmlBody) => {
  const styles = `
    <style>
      @media print {
    ...
    <html>
      ...
      <body>
        <div class="print-header">
          ...
          <div class="print-date">Printed on ${(/* @__PURE__ */ new Date()).toLocaleDateString()} at ${(/* @__PURE__ */ new Date()).toLocaleTimeString()}</div>
        </div>
...
const printBlobDocument = (blob, fileName) => {
