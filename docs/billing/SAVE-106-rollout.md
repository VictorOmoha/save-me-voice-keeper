...
Stripe is an input, not the entitlement authority. `billing_entitlements/{uid}` is the server-owned, versioned contract (`schemaVersion: 1`) containing plan, normalized status, entitlement decision, customer/subscription IDs, period end, last applied event, and mapping error. Existing `users/{uid}.subscription*` fields remain a compatibility projection written in the same transaction.
