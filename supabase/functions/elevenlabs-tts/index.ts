...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      return new Response(
        JSON.stringify({ error: 'Missing authorization header', fallback: 'browser_tts' }),
    ...
    if (!supabaseUrl || !supabaseAnonKey) {
      return new Response(
        JSON.stringify({ error: 'Server configuration error', fallback: 'browser_tts' }),
    ...
    if (authError || !user) {
      return new Response(
        JSON.stringify({ error: 'Invalid or expired token', fallback: 'browser_tts' }),
        { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
