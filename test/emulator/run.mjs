 ...
 *   4. Seeds the synthetic fixture (test/emulator/seed.ts).
...
async function main() {
  ...
  const seedCmd = `node --loader ts-node/esm "${SEED_FILE}"`;
  const testCmd = `npx vitest run "${path.join(TEST_GLOB_DIR, "*.test.ts")}" ${passthrough}`.trim();
