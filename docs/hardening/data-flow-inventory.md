...
| 13 | **Google Fonts** (`fonts.googleapis.com`, `fonts.gstatic.com`) | Font delivery on web app | Request metadata (IP, UA) inherent to font fetch (H-incidental) | **Confirmed-in-code** | No. |
| 14 | **Browser Speech APIs** (Web Speech API / SpeechSynthesis) | On-device fallback TTS and transitional speech recognition | B (utterances) processed on-device / by OS-level services [vendor routing depends on browser/OS — UNKNOWN to repo] | **Confirmed-in-code (client)** | Not disclosed. |
