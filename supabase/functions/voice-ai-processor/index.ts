...
async function authenticateUser(req: Request, corsHeaders: Record<string, string>): Promise<Response | null> {
  ...
  if (!authHeader) {
    return new Response(
      JSON.stringify({
        ...
        intent: 'unknown',
        conversationalResponse: 'Authentication required. Please log in to use voice commands.'
      }),
  ...
  if (!supabaseUrl || !supabaseAnonKey) {
    return new Response(
      JSON.stringify({
        ...
        intent: 'unknown',
        conversationalResponse: 'Server configuration error. Please try again later.'
      }),
  ...
  if (authError || !user) {
    return new Response(
      JSON.stringify({
        ...
        intent: 'unknown',
        conversationalResponse: 'Your session has expired. Please log in again.'
      }),
      { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  }
...
serve(async (req) => {
  ...
  // Authenticate user
  const authErrorResponse = await authenticateUser(req, corsHeaders);
