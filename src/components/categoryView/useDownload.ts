...
export const useDownload = () => {
  ...
  const handleDownload = async (entry: SavedEntry) => {
    ...
    try {
      if (typeof entry.fields.storagePath === 'string' && entry.fields.storagePath) {
        const {getDocumentFromStorage} = await import('@/utils/documentStorage');
        const blob = await getDocumentFromStorage(entry.fields.storagePath);
        if (!blob) throw new Error('File unavailable');
        const url = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = String(entry.fields.fileName);
      ...
      // Support files saved by older versions on this device.
