# SAVE-003 — Approved Plan Lifecycle Contract

**Status:** **RATIFIED by Victor under D-004 on 2026-08-10.** The target behavior in this document is the approved commercial contract for SAVE-105 and SAVE-106. It is not yet implemented production behavior; each section still distinguishes the current baseline from the approved target.
...
- `subscriptionTier` — the catalog plan ID (`free|basic|premium`).
...
## 9. Ratified D-004 decisions

1. Launch plans are monthly-only Free, Basic ($9), and Premium ($19). Pro, Teams, annual, and Enterprise are removed until implemented.
2. Free includes 50 entries and 500 MB; Basic includes unlimited entries and 5 GB; Premium includes unlimited entries and 50 GB.
3. There is no paid trial. Free is the no-card evaluation path.
4. Voice and portable export are universal. Basic adds browser-extension access and advanced search. Premium adds agent API access.
5. Backup, custom integrations, enhanced privacy controls, priority/24-7/dedicated support, and mobile/desktop claims are removed until real.
6. Upgrades are immediate and prorated; downgrade and cancellation take effect at period end.
7. Failed payments receive seven days of full-access grace. After grace or downgrade, over-limit resources are write-locked while existing content remains readable and exportable; nothing is silently deleted.
8. Launch payments are Stripe card only. Partial-month refunds are not offered except where legally required.
9. Stripe and server-side entitlement checks are authoritative. Unknown price IDs fail closed and alert.

---

*End of ratified SAVE-003 lifecycle contract.*
