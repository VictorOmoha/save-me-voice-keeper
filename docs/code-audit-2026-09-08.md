...
The local audit focused on account isolation, server authorization, billing, offline synchronization, and build reliability. The fixes below are implemented in this checkout. They have not been deployed.

| Priority | Finding and impact | Fix |
|---|---|---|
| High | Clients could change their `users` document, including the Stripe customer ID trusted by the billing portal and subscription entitlements. | Made the collection readable by its owner and writable only by the server. Editable profile data remains in `profiles`. |
...
| Medium | Configured premium Stripe prices fell back to basic; checkout completion did not set the purchased tier. | Use the same price configuration for checkout and webhooks. Read subscription status and price when checkout completes. Unknown prices grant no paid tier. |
...
- Default Vitest suite: **34 files, 110 tests passed**. This includes regression coverage for key scope, revocation ownership, private memories, foreign conversation IDs, CORS failures, offline account changes, queue failures, and configured billing prices.
...
- Firestore emulator: **14 checks passed**, including denial of billing edits/deletion, ownership transfer, unauthorized entry deletion, and direct key mutation; owner reads and profile edits remain allowed.
- Production build: passed with minification. Remaining build warnings concern old Browserslist data and the document-viewer chunk exceeding 500 kB.
...
Deploy the updated Firestore rules and Cloud Functions, including `sharedMemoryRevokeAgentKey`, before publishing the updated frontend. No live Firebase data, Stripe subscription, or agent memory was changed during this audit. Existing integrations using agent keys outside shared-memory endpoints will now need a Firebase user session. Selected-agent visibility grants are not implemented; API keys cannot access those records.

The public npm advisory scan completed after the user approved exporting dependency names and versions. The final `--omit=dev` scans report **2 moderate frontend findings and 8 moderate backend findings**, with no high/critical findings. The counts include packages affected transitively by the same underlying advisory; they are not ten independent exploits. Full registry findings are saved in `docs/dependency-audit-2026-09-08.json`.

The remaining frontend findings require React Router 7.18 or later. Backend findings trace to older UUID dependencies in Firebase/Google Cloud libraries. npm proposes Firebase Admin 14, which also requires Node 22 and removes the namespace API used throughout this backend, according to the [official release notes](https://firebase.google.com/support/release-notes/admin/node). These major-version migrations remain open; no forced upgrade or major transitive override was applied. This audit does not claim that all dependencies are vulnerability-free. Live voice, payment, and extension flows were not exercised against production services.
